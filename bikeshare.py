...
if isinstance(city, list):
  df = pd.concat(map(lambda city: pd.read_csv(city_data[city]), city),
    sort = True)# reorganize DataFrame columns after a city concat
try:
...
print(" the most common start station is: " + mcommon_start_station)# TO DO: display most commonly used end station
mcommon_end_station = df['End Station'].mode()[0]
print(" the most common start end is: " + mcommon_end_station)# TO DO: display most frequent combination of start station and end station trip
...
total_travel_time = (str(int(total_travel_time //86400)) + 'd ' + str(int((total_travel_time % 86400)//3600)) +'h '+str(int(((total_travel_time % 86400) % 3600)//60)) +'m ' + str(int(((total_travel_time % 86400) % 3600) % 60)) +'s')
        ...
        # TO DO: display mean travel time mean_travel_time = df['Trip Duration'].mean() mean_travel_time = (str(int(mean_travel_time //60)) + 'm ' + str(int(mean_travel_time % 60)) + 's')
              print("the mean travel time is : " + mean_travel_time + ".") print("\nThis took %s seconds." % (time.time() - start_time))
              ...
              # TO DO: Display counts of user types user_types = df['User Type'].value_counts() print('User type distribution:', user_types) try: #TO DO: Display counts of gender genders = df['Gender'].value_counts() print('Gender distribution', genders) except KeyError:
              print(" for this City has no data for gender distribution")

              # TO DO: Display earliest, most recent, and most common year of birth
              try:
              earliest_birth = df['Birth Year'].min() print('the oldest person: ', earliest_birth) most_recent_birth = df['Birth Year'].max() print('the youngest person:', most_recent_birth) common_birth = df['Birth Year'].mode() print('common birthday:', common_birth) except KeyError:
              ...
              # each loop displays 5 lines of raw data global from_offset
              ...
              while True:
              raw_data = df.iloc[from_offset: from_offset + 5] print(raw_data) user_input = input("Do you still want to keep printing raw data?\n\n[y]Yes\n[n]No\n\n>") if user_input in ['y', 'yes', 'YES', 'Yes']:
              ...
              def main():
              while True:
              city, month, day = get_filters() df = load_data(city, month, day)

              time_stats(df) station_stats(df) trip_duration_stats(df) user_stats(df) raw_data = slicing(df)
